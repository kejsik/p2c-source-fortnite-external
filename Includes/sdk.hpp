...
{
	uintptr_t bone_array = drv->Read<uintptr_t>(mesh + 0x600);
	if (bone_array == NULL) bone_array = drv->Read<uintptr_t>(mesh + 0x600 + 0x10);
	FTransform bone = drv->Read<FTransform>(bone_array + (bone_id * 0x60));
	FTransform component_to_world = drv->Read<FTransform>(mesh + 0x240);
...
{
	float last_sumbit_time = drv->Read<float>(mesh + 0x360);
	float last_render_time_on_screen = drv->Read<float>(mesh + 0x368);
...
{
	char v1; // r8
	CamewaDescwipsion ViewPoint = drv->Read<CamewaDescwipsion>(baseaddy + 0xF0E8BD0);
...
std::string find_object_class(int key)
{
	...
	uint64_t NamePoolChunk = drv->Read<uint64_t>(base_address + 0x10376500 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); //((ChunkOffset + 2) * 8) ERROR_NAME_SIZE_EXCEEDED
	...
	if ((uint32_t)nameLength)
	{
		...
		{
			buff[x] = drv->Read<char>(NamePoolChunk + 4 + x);
		}
		...
		v5 = drv->Read<int>(base_address + 0x102CE0A8);
		v6 = (v5 << 8) | (v5 >> 8);
		v7 = v5 >> 15;
		if (v4) {
			v8 = v4;
...
std::string find_object_name(int key)
{
	...
	uint64_t NamePoolChunk = drv->Read<uint64_t>(base_address + 0x10376500 + (8 * ChunkOffset) + 16) + (unsigned int)(4 * NameOffset); // ERROR_NAME_SIZE_EXCEEDED
